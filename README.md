...
1. Add the complete question and your answer. Each field has **Type / paste** and **Photos**, so you can mix methods. The Question → Answer → Feedback strip shows what is ready and links to each step. Text tables, statistical symbols, calculations, and multipart answers are accepted.
...
You can also drop images onto the photo area or paste a screenshot while a photo-area control is focused. **Type from these photos** lets you transcribe manually without sending the photos to the AI provider. The review area shows the original beside the editable text on wider screens, with page navigation; narrow screens stack them. `[unclear]` markers are counted to help students check unreadable portions without guessing. An **Undo** action restores the exact pre-import text while the imported text is still unchanged.
...
Typed work and pending photos stay in memory while opening Saved reports or a sample and returning to the workspace. Starting a revision replaces that editor with the selected report's question and answer and clears its old photo drafts. A full reload still clears unsaved work; this update does not add automatic storage of student submissions. When the AI service is not configured, a prominent Sample mode notice offers sample feedback and disables automatic reading instead of allowing an avoidable failed request. Manual preparation remains available.
